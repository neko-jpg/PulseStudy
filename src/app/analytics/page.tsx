...
import { ShareBar } from '@/components/analytics/ShareBar'
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <h1 className="text-lg font-semibold">学習分析</h1>
        <div className="text-xs text-muted-foreground">要点の把握 → 次の一手 → 共有。迷わず進めます。</div>
      </div>
      ...
      {!loading && error && (
        <Card className="p-4 text-sm">読み込みに失敗しました。<button className="underline ml-2" onClick={() => location.reload()}>再試行</button></Card>
      )}
      ...
      {data && (
        <>
          <SummaryCards
            ...
            onOpen={(m) => { setModal('summary'); track({ name: 'analytics_card_open', props: { metric: m } }) }}
          />

          <Card className="p-4">
            <div className="font-semibold mb-2">ヒートマップ</div>
            <Heatmap cells={data.heatmap} onClick={fetchLogs} />
            {activeDate && (
              <div className="mt-3">
                <div className="text-sm font-medium">{activeDate} のログ</div>
          ...
          <Card className="p-4">
            <ShareBar onShare={(url) => console.log('share url', url)} />
          </Card>

          <Dialog open={modal === 'summary'} onOpenChange={(o) => setModal(o ? 'summary' : 'none')}>
            <DialogContent>
              <DialogHeader><DialogTitle>サマリー詳細</DialogTitle></DialogHeader>
              <div className="text-sm space-y-1">
                {trends.slice(-7).map((t) => (
                  <div key={t.date} className="flex justify-between">
                    <span>{t.date}</span><span>{t.mins}分</span><span>{Math.round(t.acc * 100)}%</span><span>{t.flow}%</span>
                  </div>
                ))}
              </div>
            </DialogContent>
